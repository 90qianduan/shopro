/* vuex 数据常量 */

export const INIT_DATA = 'INIT_DATA' //初始化配置数据
export const PAGE_ROUTES = 'PAGE_ROUTES' //前端路由表
export const TEMPLATE_DATA = 'TEMPLATE_DATA' //初始化模板信息
export const USER_INFO = 'USER_INFO' //用户信息
export const ORDER_NUMBER = 'ORDER_NUMBER' //订单数量，角标
export const OUT_LOGIN = 'OUT_LOGIN' //退出登录
export const CART_LIST = 'CART_LIST' //购物车商品数据
export const CART_NUM = 'CART_NUM' //购物车商品数
export const LOGIN_TIP = 'LOGIN_TIP' //登录提示
// #ifdef MP-WEIXIN
export const FORCE_OAUTH = 'FORCE_OAUTH' // 小程序强制登录弹窗
// #endif
export const MESSAGE_IDS = 'MESSAGE_IDS' // 小程序订阅消息模板ids
